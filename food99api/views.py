...
class RegisterView(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            client = Brevo(settings.brevo_ID)
            ...
            def send_otp_email():
                ...
                subject="OTP - Confirm to Login in TradeB2B Account!",
                html_content=f"<html><body><p>Hello,</p><p>This is your one time password <B>{otp}</B>. Use this code to secure sign-in in your tradeb2b.online account.</p></body></html>",
                sender=SendTransacEmailRequestSender(
                    name="One Time Password from TradeB2B.online",
...
class VerifyOtpView(APIView):
    def post(self, request, otp_uuid, username_, *args, **kwargs):
        code = request.data.get("otp")
        try:
            user = CustomUser.objects.get(username=username_)
    ...
    def get(self, request, otp_uuid, username_):
        try:
            user = CustomUser.objects.get(username=username_)
            otp_obj = Otp.objects.get(user=user, uuid=otp_uuid)
